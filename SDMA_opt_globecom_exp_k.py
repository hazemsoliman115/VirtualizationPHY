# -*- coding: utf-8 -*-
"""
Created on Mon Mar 28 17:56:42 2016
...
if __name__ == "__main__":
    ...
    exp_user_list = [i for i in range(7)]
    total_slice_no = [[[0 for i in range(len(Num_slices_list))] for k in range(len(exp_user_list))]for j in range(len(Q_epsilon_list))]
    total_slice_no_SDMA = [[[0 for i in range(len(Num_slices_list))] for k in range(len(exp_user_list))]for j in range(len(Q_epsilon_list))]
    for i_num_slice in range(len(Num_slices_list)):
        print(i_num_slice)
        for i_exp in range(len(exp_user_list)):
            for i_Q_eps in range(len(Q_epsilon_list)):
                for k in range(no_iter_statistics):
                    #Num_slices = 5
                    Num_slices = Num_slices_list[i_num_slice]
                    NumRes = 10
                    exp_users = exp_user_list[i_exp]
                    total_antenna  = 8
                    per_slice_antenna = 4
                    QoS_expo = 0.95
                    sigma = 0.2
                    ...
                    total_slice_no[i_Q_eps][i_exp][i_num_slice] += len(list_slices_final)
                    
                    total_slice_no_SDMA[i_Q_eps][i_exp][i_num_slice] += temp/len(list_RBs)
                    
                total_slice_no[i_Q_eps][i_exp][i_num_slice] = total_slice_no[i_Q_eps][i_exp][i_num_slice]/no_iter_statistics
                total_slice_no_SDMA[i_Q_eps][i_exp][i_num_slice] = total_slice_no_SDMA[i_Q_eps][i_exp][i_num_slice]/no_iter_statistics
    marker_list='osv<>xd'
    for i in range(len(exp_user_list)):
        plt.plot(Q_epsilon_list,[total_slice_no[j][i][0] for j in range(len(Q_epsilon_list))], marker = marker_list[i], label = 'E(k)='+str(exp_user_list[i]))
        plt.axis([0.5, 6, 2.5, 6.5])
        plt.legend(loc=1)
        plt.xlabel('$\epsilon$')
        plt.ylabel('Number of Selected Slices')
        plt.title('Number of Selected Slices for varying E(k)')
    plt.savefig('NumberSlices28032016_E_k.pdf', bbox_inches='tight')
    plt.savefig('NumberSlices28032016_E_k.eps', bbox_inches='tight')
    marker_list='osv<>xd'
    for i in range(len(exp_user_list)):
        plt.plot(Q_epsilon_list,[total_slice_no_SDMA[j][i][0] for j in range(len(Q_epsilon_list))], marker = marker_list[i], label = 'E(k)='+str(exp_user_list[i]))
        plt.axis([0.5, 6, 0.6, 2])
        plt.legend(loc=1)
        plt.xlabel('$\epsilon$')
        plt.ylabel('Number of Slices Per Frequency Resource')
        plt.title('Number of Multiplexed Slices for varying E(k)')
    plt.savefig('NumberSlices28032016SDMA_E_k.pdf', bbox_inches='tight')
    plt.savefig('NumberSlices28032016SDMA_E_k.eps', bbox_inches='tight')
